from cvec_circus.process import Process, debuglog
from cvec_circus.util import papa
from cvec_circus import logger
import psutil
import select
import time
from cvec_circus.py3compat import PY2, string_types
...
class PapaProcessProxy(Process):
    ...
    def _fix_socket_name(self, s, socket_names):
        if s:
            s_lower = s.lower()
            while '$(cvec_circus.sockets.' in s_lower:
                start = s_lower.index('$(cvec_circus.sockets.')
                ...
                s = ''.join((s[:start], '$(socket.cvec_circus.', socket_name,
                             '.fileno)', s[end + 1:]))
                s_lower = s.lower()
        return s
    ...
    def spawn(self):
        ...
        papa_name = 'cvec_circus.{0}.{1}'.format(self.name, self.wid).lower()

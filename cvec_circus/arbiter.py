...
import os
import gc
from cvec_circus.fixed_threading import Thread, get_ident
import sys
import select
import socket
...
from cvec_circus.controller import Controller
from cvec_circus.exc import AlreadyExist
from cvec_circus import logger
from cvec_circus.watcher import Watcher
from cvec_circus.util import debuglog, _setproctitle, parse_env_dict
from cvec_circus.util import DictDiffer, synchronized, tornado_sleep, papa
from cvec_circus.util import IS_WINDOWS
from cvec_circus.config import get_config
from cvec_circus.plugins import get_plugin_cmd
from cvec_circus.sockets import CircusSocket, CircusSockets
...
class Arbiter(object):
    ...
    - **statsd** -- If True, a cvec_circusd-stats process is run (default: False)
    - **stats_endpoint** -- the stats endpoint.
    - **statsd_close_outputs** -- if True sends the cvec_circusd-stats stdout/stderr
      to /dev/null (default: False)
    - **multicast_endpoint** -- the multicast endpoint for cvec_circusd cluster
    ...
    - **warmup_delay** -- a delay in seconds between two watchers startup.
      (default: 0)
    - **httpd** -- If True, a cvec_circushttpd process is run (default: False)
    - **httpd_host** -- the cvec_circushttpd host (default: localhost)
    - **httpd_port** -- the cvec_circushttpd port (default: 8080)
    - **httpd_close_outputs** -- if True, sends cvec_circushttpd stdout/stderr
      to /dev/null. (default: False)
    - **debug** -- if True, adds a lot of debug info in the stdout (default:
      False)
    - **debug_gc** -- if True, does gc.set_debug(gc.DEBUG_LEAK) (default:
      False)
      to cvec_circusd to analyze problems (default: False)
    - **proc_name** -- the arbiter process name
    - **fqdn_prefix** -- a prefix for the unique identifier of the cvec_circus
    ...
    def __init__(self, watchers, endpoint, pubsub_endpoint, check_delay=1.0,
                 ...
                 ssh_server=None, proc_name='cvec_circusd', pidfile=None,
        ...
        # initializing cvec_circusd-stats as a watcher when configured
        self.statsd = statsd
        self.stats_endpoint = stats_endpoint

        if self.statsd:
            cmd = "%s -c 'from cvec_circus import stats; stats.main()'" % \
                sys.executable
            ...
            stats_watcher = Watcher('cvec_circusd-stats', cmd, use_sockets=True,
                                    singleton=True,
        ...
        if httpd:
            # adding the socket
            httpd_socket = CircusSocket(name='cvec_circushttpd', host=httpd_host,
                                        port=httpd_port)
            if sockets is None:
            ...
            cmd = ("%s -c 'from cvec_circusweb import cvec_circushttpd; "
                   "cvec_circushttpd.main()'") % sys.executable
            cmd += ' --endpoint %s' % self.endpoint
            cmd += ' --fd $(cvec_circus.sockets.cvec_circushttpd)'
            if ssh_server is not None:
                cmd += ' --ssh %s' % ssh_server

            # Adding the watcher
            httpd_watcher = Watcher('cvec_circushttpd', cmd, use_sockets=True,
                                    singleton=True,
                                    stdout_stream=self.stdout_stream,
                                    stderr_stream=self.stderr_stream,
                                    copy_env=True, copy_path=True,
    ...
    @gen.coroutine
    def reload_from_config(self, config_file=None, inside_cvec_circusd=False):
        ...
        if self.get_arbiter_config(new_cfg) != self._cfg:
            yield self._restart(inside_cvec_circusd=inside_cvec_circusd)
            return

        ignore_sn = set(['cvec_circushttpd'])
        ignore_wn = set(['cvec_circushttpd', 'cvec_circusd-stats'])
        ...
        for n in maybechanged_sn:
            ...
            if self.get_socket_config(new_cfg, n) != s._cfg:
                ...
                for w in self.iter_watchers():
                    if 'cvec_circus.sockets.%s' % n.lower() in w.cmd:
    ...
    def load_from_config(cls, config_file, loop=None):
        ...
        if httpd:
            ...
            try:
                import cvec_circusweb  # NOQA
            except ImportError:
                logger.error('You need to install cvec_circus-web')
                sys.exit(1)
    ...
    def initialize(self):
        ...
        # set umask even though we may have already set it early in cvec_circusd.py
    ...
    def _emergency_stop(self):
        """Emergency and fast stop, to use only in cvec_circusd
        """
        for watcher in self.iter_watchers():
    ...
    @gen.coroutine
    def _restart(self, inside_cvec_circusd=False, watcher_iter_func=None):
        if inside_cvec_circusd:
    ...
    @gen.coroutine
    def restart(self, inside_cvec_circusd=False, watcher_iter_func=None):
        yield self._restart(inside_cvec_circusd=inside_cvec_circusd,

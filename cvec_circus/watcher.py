...
from cvec_circus.process import Process, DEAD_OR_ZOMBIE, UNEXISTING
from cvec_circus.papa_process_proxy import PapaProcessProxy
from cvec_circus import logger
from cvec_circus import util
from cvec_circus.stream import get_stream, Redirector
from cvec_circus.stream.papa_redirector import PapaRedirector
from cvec_circus.util import parse_env_dict, resolve_name, tornado_sleep, IS_WINDOWS
from cvec_circus.util import papa
from cvec_circus.py3compat import bytestring, is_callable, b, PY2
...
class Watcher(object):
    ...
    - **stderr_stream**: a mapping that defines the stream for
      ...
      three keys:
      - **class**: the stream class. Defaults to `cvec_circus.stream.FileStream`
      - **filename**: the filename, if using a FileStream
      - **max_bytes**: maximum file size, after which a new output file is
        opened. defaults to 0 which means no maximum size (only applicable
        with FileStream)
    ...
    - **use_sockets** -- If True, the processes will inherit the file
      descriptors, thus can reuse the sockets opened by cvec_circusd.
      (default: False)
    ...
    - **copy_env** -- If True, the environment in which cvec_circus is running
    ...
    - **copy_path** -- If True, cvec_circusd *sys.path* is sent to the
    ...
    def __init__(self, name, cmd, args=None, numprocesses=1, warmup_delay=0.,
        ...
        if self.use_papa:
            with papa.Papa() as p:
                base_name = 'cvec_circus.{0}.*'.format(name.lower())
    ...
    def __repr__(self):
        return "<cvec_circus.Watcher name=%s numprocesses=%s>" % (self.name,
                                                             self.numprocesses)

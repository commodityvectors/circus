...
class WatchDog(CircusPlugin):
    ...
    materialized by loop_rate * max_count. (cvec_circus will automatically restart
    ...
    **watchers_regex** in the configuration.
    ...
    At startup, the plugin does not know all the cvec_circus watchers and pids,
    so it's needed to discover all watchers and pids. After the discover, the
    monitoring list is updated by messages from cvec_circusd handled in
    self.handle_recv
    ...
    def handle_recv(self, data):
        """Handle received message from cvec_circusd
        ...
        logger.debug("received data from cvec_circusd: watcher.%s.%s, %s",
    ...
    def _discover_monitored_pids(self):
        ...
        from cvec_circusd which is handled by self.handle_recv
        """
        self.pid_status = dict()

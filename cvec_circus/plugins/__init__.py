...
from cvec_circus import logger, __version__
from cvec_circus.client import make_message, cast_message
from cvec_circus.py3compat import b, s
from cvec_circus.util import (debuglog, to_bool, resolve_name, configure_logger,
...
class CircusPlugin(object):
    ...
    - **endpoint** -- the cvec_circusd ZMQ endpoint
    - **pubsub_endpoint** -- the cvec_circusd ZMQ pub/sub endpoint
    ...
    def call(self, command, **props):
        """Sends the command to **cvec_circusd**
        ...
        Returns the JSON mapping sent back by **cvec_circusd**
        """
    ...
    def cast(self, command, **props):
        """Fire-and-forget a command to **cvec_circusd**
    ...
    def handle_recv(self, data):
        """Receives every event published by **cvec_circusd**
...
def get_plugin_cmd(config, endpoint, pubsub, check_delay, ssh_server,
    ...
    cmd = "%s -c 'from cvec_circus import plugins;plugins.main()'" % sys.executable
...
def main():
    ...
    parser.add_argument('--endpoint',
                        help='The cvec_circusd ZeroMQ socket to connect to',
                        default=DEFAULT_ENDPOINT_DEALER)

    parser.add_argument('--pubsub',
                        help='The cvec_circusd ZeroMQ pub/sub socket to connect to',

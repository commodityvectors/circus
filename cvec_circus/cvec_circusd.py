...
from cvec_circus.pidfile import Pidfile
from cvec_circus import __version__
from cvec_circus.util import MAXFD, REDIRECT_TO, configure_logger, LOG_LEVELS
...
def main():
    ...
    if zmq_version[0] < 13 or (zmq_version[0] == 13 and zmq_version[1] < 1):
        print('cvec_circusd needs PyZMQ >= 13.1.0 to run - aborting...')
        sys.exit(0)
    ...
    parser.add_argument('--daemon', dest='daemonize', action='store_true',
                        help="Start cvec_circusd in the background. Not supported "
                             "on Windows")

...
from cvec_circus.client import AsyncCircusClient
from cvec_circus.plugins import CircusPlugin
from cvec_circus.tests.support import (TestCircus, async_poll_for, truncate_file,
                                  EasyTestSuite, skipIf, get_ioloop, SLEEP,
                                  PYTHON)
from cvec_circus.util import (DEFAULT_ENDPOINT_DEALER, DEFAULT_ENDPOINT_MULTICAST,
                         DEFAULT_ENDPOINT_SUB)
from cvec_circus.tests.support import (MockWatcher, has_cvec_circusweb,
                                  poll_for_callable, get_available_port)
from cvec_circus import watcher as watcher_mod
...
class TestTrainer(TestCircus):
    ...
    def _get_cmd(self):
        ...
        cmd = '%s %s %s %s' % (
            PYTHON, _GENERIC,
            'cvec_circus.tests.support.run_process',
            testfile)
    ...
    def _get_cmd_args(self):
        cmd = PYTHON
        args = [_GENERIC, 'cvec_circus.tests.support.run_process']
        return cmd, args
    ...
    def test_plugins(self):
        ...
        # setting up a cvec_circusd with a plugin
        plugin = 'cvec_circus.tests.test_arbiter.Plugin'
    ...
    def _test_udp_discovery(self):
        """test_udp_discovery: Test that when the cvec_circusd answer UDP call.

        """
        yield self._stop_runners()

        dummy_process = 'cvec_circus.tests.support.run_process'
        self._run_cvec_circus(dummy_process)
...
@skipIf(not has_cvec_circusweb(), 'Tests for cvec_circus-web')
class TestCircusWeb(TestCircus):

    @tornado.testing.gen_test
    def test_cvec_circushttpd(self):
        controller = "tcp://127.0.0.1:%d" % get_available_port()
        sub = "tcp://127.0.0.1:%d" % get_available_port()
        ...
        try:
            ...
            poll_for_callable(self.assertDictEqual,
                              arbiter.statuses, {'cvec_circushttpd': 'active'})
        finally:
            yield arbiter.stop()

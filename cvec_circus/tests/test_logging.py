...
from cvec_circus.tests.support import TestCase
from cvec_circus.tests.support import EasyTestSuite
from cvec_circus.tests.support import skipIf, PYTHON, IS_WINDOWS
import os
import shutil
import tempfile
import subprocess
...
CONFIG_PATH = os.path.join(HERE, 'config', 'cvec_circus.ini')


def run_cvec_circusd(options=(), config=(), log_capture_path="log.txt",
                additional_files=()):
    ...
    try:
        cvec_circus_ini_path = os.path.join(temp_dir, "cvec_circus.ini")
        with open(cvec_circus_ini_path, "w") as fh:
            config_ini.write(fh)
        ...
        # We're going to run cvec_circus from a process with a different
        # cwd, so we need to make sure that Python will import the
        # current version of cvec_circus
        pythonpath = env.get('PYTHONPATH', '')
        pythonpath += '%s%s' % (sep, os.path.abspath(
            os.path.join(HERE, os.pardir, os.pardir)))
        env['PYTHONPATH'] = pythonpath
        argv = ["cvec_circus.cvec_circusd"] + options + [cvec_circus_ini_path]
        ...
        try:
            ...
            # then, make sure that cvec_circus started and the log are going
...
class TestLoggingConfig(TestCase):
    ...
    def test_loggerconfig_default_ini(self):
        run_cvec_circusd(
            [], {"cvec_circus.logoutput": "log_ini.txt"},
    ...
    def test_loggerconfig_yaml_ini(self):
        ...
        run_cvec_circusd(
            [], {"cvec_circus.loggerconfig": "logging.yaml"},
    ...
    def test_loggerconfig_json_ini(self):
        ...
        run_cvec_circusd(
            [], {"cvec_circus.loggerconfig": "logging.json"},
    ...
    def test_loggerconfig_ini_ini(self):
        ...
        run_cvec_circusd(
            [], {"cvec_circus.loggerconfig": "logging.ini"},

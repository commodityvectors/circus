...
from cvec_circus.client import AsyncCircusClient, make_message
from cvec_circus.util import DEFAULT_ENDPOINT_DEALER, DEFAULT_ENDPOINT_SUB
from cvec_circus.util import tornado_sleep, ConflictError
...
class TestCircus(AsyncTestCase):
    ...
    def start_arbiter(self, cmd='support.run_process',
        ...
        testfile, arbiter = self._create_cvec_circus(
    ...
    @classmethod
    def _create_cvec_circus(cls, callable_path, plugins=None, stats=False,
                       async=False, arbiter_kw=None, **kw):
        fd, testfile = mkstemp()
        os.close(fd)
        wdir = os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.realpath(__file__))))
        args = ['cvec_circus/tests/generic.py', callable_path, testfile]
...
def async_run_plugin(klass, config, plugin_info_callback, duration=300,
    ...
    cvec_circusctl_process = multiprocessing.Process(
        target=run_plugin,

...
from cvec_circus.tests.support import TestCircus, EasyTestSuite, TimeoutException
from cvec_circus.tests.support import skipIf, IS_WINDOWS
from cvec_circus.client import AsyncCircusClient
from cvec_circus.stream import QueueStream, Empty
...
class SignalCommandTest(TestCircus):
    ...
    def test_handler(self):
        stream = QueueStream()
        cmd = 'cvec_circus.tests.test_command_signal.run_process'
...
class SignalRecursiveCommandTest(TestCircus):
    ...
    def test_handler(self):
        stream = QueueStream()
        cmd = 'cvec_circus.tests.test_command_signal.run_process_recursive'
        stdout_stream = {'stream': stream}
        stderr_stream = {'stream': stream}
        yield self.start_arbiter(cmd=cmd, stdout_stream=stdout_stream,
                                 stderr_stream=stderr_stream, stats=True,

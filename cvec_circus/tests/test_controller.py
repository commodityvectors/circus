from cvec_circus.tests.support import TestCase, EasyTestSuite, get_ioloop
from cvec_circus.controller import Controller
from cvec_circus.util import DEFAULT_ENDPOINT_MULTICAST
from cvec_circus import logger
import cvec_circus.controller
...
class TestController(TestCase):
    ...
    def _multicast_side_effect_helper(self, side_effect):
        ...
        controller = cvec_circus.controller.Controller(
            'endpoint', DEFAULT_ENDPOINT_MULTICAST, context, loop, arbiter
        )

        with mock.patch('cvec_circus.util.create_udp_socket') as m:
            m.side_effect = side_effect
            cvec_circus.controller.create_udp_socket = m

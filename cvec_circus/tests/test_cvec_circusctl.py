...
from cvec_circus.cvec_circusctl import USAGE, VERSION, CircusCtl
from cvec_circus.tests.support import (TestCircus, async_poll_for, EasyTestSuite,
                                  skipIf, DEBUG, PYTHON, SLEEP)
from cvec_circus.util import tornado_sleep, DEFAULT_ENDPOINT_DEALER
from cvec_circus.py3compat import b, s


def run_ctl(args, queue=None, stdin='', endpoint=DEFAULT_ENDPOINT_DEALER):
    cmd = '%s -m cvec_circus.cvec_circusctl' % PYTHON
...
def async_run_ctl(args, stdin='', endpoint=DEFAULT_ENDPOINT_DEALER):
    """
    Start a process that will start the actual cvec_circusctl process and poll its
    ouput, via a queue, without blocking the I/O loop. We do this to avoid
    blocking the main thread while waiting for cvec_circusctl output, so that the
    arbiter will be able to respond to requests coming from cvec_circusctl.
    """
    queue = Queue()
    cvec_circusctl_process = Process(target=run_ctl,
                                args=(args, queue, stdin,
                                      endpoint))
    cvec_circusctl_process.start()
    while queue.empty():
...
class CommandlineTest(TestCircus):
    ...
    def test_help_invalid_command(self):
        ...
        self.assertEqual(err[1],
                         'cvec_circusctl.py: error: unrecognized arguments: foo')

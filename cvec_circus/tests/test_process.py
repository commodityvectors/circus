...
from cvec_circus.tests.support import (TestCircus, skipIf, EasyTestSuite, DEBUG,
                                  poll_for, IS_WINDOWS, PYTHON, SLEEP)
import cvec_circus.py3compat
from cvec_circus.py3compat import StringIO, PY2
...
class TestProcess(TestCircus):
    ...
    def test_rlimits(self):
        ...
        def srt2ints(val):
            return [cvec_circus.py3compat.long(key) for key in val[1:-1].split(',')]

        wanted = [cvec_circus.py3compat.long(20), cvec_circus.py3compat.long(20)]
    ...
    def test_process_parameters(self):
        ...
        p1 = Process('test', 1, 'make-me-a-coffee',
                     '$(cvec_circus.wid) --type $(cvec_circus.env.type)',
        ...
        p2 = Process('test', 1, 'yeah $(CVEC_CIRCUS.WID)', spawn=False,
                     use_fds=USE_FDS)
        self.assertEqual(['yeah', '1'], p2.format_args())

        os.environ['coffee_type'] = 'american'
        p3 = Process('test', 1, 'yeah $(cvec_circus.env.type)', shell=False,

...
from cvec_circus.tests.support import TestCircus, EasyTestSuite, skipIf, IS_WINDOWS
from cvec_circus.client import AsyncCircusClient
from cvec_circus.stream import FileStream
from cvec_circus.py3compat import get_next
from cvec_circus.util import tornado_sleep
...
class TestStatsClient(TestCircus):
    ...
    def test_handler(self):
        ...
        cmd = 'cvec_circus.tests.test_stats_client.run_process'
        ...
        self.assertEqual(['cvec_circusd-stats', 'test'], watchers)

        # making sure the stats process run
        res = yield client.send_message('status', name='test')
        self.assertEqual(res['status'], 'active')

        res = yield client.send_message('status', name='cvec_circusd-stats')
        self.assertEqual(res['status'], 'active')

        # playing around with the stats now: we should get some !
        from cvec_circus.stats.client import StatsClient
        ...
        for i in range(10):
            ...
            self.assertTrue(watcher in ('test', 'cvec_circusd-stats', 'cvec_circus'),
                            watcher)

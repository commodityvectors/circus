...
from cvec_circus.tests.support import (TestCase, EasyTestSuite, skipIf,
                                  IS_WINDOWS, SLEEP)

from cvec_circus import util
from cvec_circus.util import (
...
class TestUtil(TestCase):
    ...
    def test_replace_gnu_args(self):
        ...
        self.assertEqual('dont change --fd ((cvec_circus.me)) please',
                         repl('dont change --fd ((cvec_circus.me)) please'))

        self.assertEqual('dont change --fd $(cvec_circus.me) please',
                         repl('dont change --fd $(cvec_circus.me) please'))

        self.assertEqual('thats an int 2',
                         repl('thats an int $(cvec_circus.me)',
                              me=2))

        self.assertEqual('foobar', replace_gnu_args('$(cvec_circus.test)',
                         test='foobar'))
        self.assertEqual('foobar', replace_gnu_args('$(cvec_circus.test)',
                         test='foobar'))
        self.assertEqual('foo, foobar, baz',
                         replace_gnu_args('foo, $(cvec_circus.test), baz',
                                          test='foobar'))
        self.assertEqual('foo, foobar, baz',
                         replace_gnu_args('foo, ((cvec_circus.test)), baz',
                                          test='foobar'))
    ...
    @mock.patch('cvec_circus.util.os.environ', {'PWD': '/path/to/pwd'})
    @mock.patch('cvec_circus.util.os.getcwd', lambda: '/path/to/cwd')

...
logging.basicConfig()
logger = logging.getLogger('cvec_circus')
...
class ArbiterHandler(object):
    def __call__(self, watchers, controller=None,
                 ...
                 plugins=None, debug=False, proc_name="cvec_circusd",
            ...
            - **stdout_stream**: a mapping containing the options for
              ...
              the stdout stream. Default to None. When provided, may contain:
                ...
                - **class**: the fully qualified name of the class to use for
                             streaming. Defaults to cvec_circus.stream.FileStream
        ...
        - **stats_endpoint** -- the stats endpoint. If not provided,
          the *cvec_circusd-stats* process will not be launched. (default: None)
        - **statsd_close_outputs** -- if True sends the cvec_circusd-stats
        ...
        - **debug** -- If True the arbiter is launched in debug mode
          (default: False)
        - **proc_name** -- the arbiter process name (default: cvec_circusd)
    ...
    def _get_arbiter_klass(self, background):
        if background:
            from cvec_circus.arbiter import ThreadedArbiter as Arbiter   # NOQA
        else:
            from cvec_circus.arbiter import Arbiter   # NOQA
        return Arbiter

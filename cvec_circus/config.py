...
from cvec_circus.py3compat import sort_by_field
from cvec_circus.util import (DEFAULT_ENDPOINT_DEALER, DEFAULT_ENDPOINT_SUB,
...
def read_config(config_path):
    ...
    for include_file in cfg.dget('cvec_circus', 'include', '').split():
        _scan(include_file, includes)

    for include_dir in cfg.dget('cvec_circus', 'include_dir', '').split():
...
def get_config(config_file):
    ...
    # main cvec_circus options
    config['check_delay'] = dget('cvec_circus', 'check_delay', 5., float)
    config['endpoint'] = dget('cvec_circus', 'endpoint', DEFAULT_ENDPOINT_DEALER)
    config['endpoint_owner'] = dget('cvec_circus', 'endpoint_owner', None, str)
    config['pubsub_endpoint'] = dget('cvec_circus', 'pubsub_endpoint',
                                     DEFAULT_ENDPOINT_SUB)
    config['multicast_endpoint'] = dget('cvec_circus', 'multicast_endpoint',
                                        DEFAULT_ENDPOINT_MULTICAST)
    config['stats_endpoint'] = dget('cvec_circus', 'stats_endpoint', None)
    config['statsd'] = dget('cvec_circus', 'statsd', False, bool)
    config['umask'] = dget('cvec_circus', 'umask', None)
    if config['umask']:
    ...
    config['warmup_delay'] = dget('cvec_circus', 'warmup_delay', 0, int)
    config['httpd'] = dget('cvec_circus', 'httpd', False, bool)
    config['httpd_host'] = dget('cvec_circus', 'httpd_host', 'localhost', str)
    config['httpd_port'] = dget('cvec_circus', 'httpd_port', 8080, int)
    config['debug'] = dget('cvec_circus', 'debug', False, bool)
    config['debug_gc'] = dget('cvec_circus', 'debug_gc', False, bool)
    config['pidfile'] = dget('cvec_circus', 'pidfile')
    config['loglevel'] = dget('cvec_circus', 'loglevel')
    config['logoutput'] = dget('cvec_circus', 'logoutput')
    config['loggerconfig'] = dget('cvec_circus', 'loggerconfig', None)
    config['fqdn_prefix'] = dget('cvec_circus', 'fqdn_prefix', None, str)
    config['papa_endpoint'] = dget('cvec_circus', 'fqdn_prefix', None, str)

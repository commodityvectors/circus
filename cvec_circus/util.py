...
from cvec_circus.py3compat import (
...
from cvec_circus.exc import ConflictError
from cvec_circus import logger
from cvec_circus.py3compat import string_types
...
def get_info(process=None, interval=0, with_childs=False):
    ...
    """
    # XXX moce get_info to cvec_circus.process ?
    from cvec_circus.process import (get_children, get_memory_info,
...
_CIRCUS_VAR = re.compile(_PATTERN1 % 'cvec_circus' + '|' +
                         _PATTERN2 % 'cvec_circus', re.I)


def replace_gnu_args(data, prefix='cvec_circus', **options):
    fmt_options = {}
...
def create_udp_socket(mcast_addr, mcast_port):
    """Create an udp multicast socket for cvec_circusd cluster auto-discovery.

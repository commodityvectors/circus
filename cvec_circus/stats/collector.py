...
class WatcherStatsCollector(BaseStatsCollector):
    ...
    def collect_stats(self):
        ...
        for pid in self.streamer.get_pids(self.name):
            ...
            if self.name == 'cvec_circus':
                if pid in self.streamer.cvec_circus_pids:
                    name = self.streamer.cvec_circus_pids[pid]
...
# socket, so the cvec_circusd-stats process don't eat all your CPU
